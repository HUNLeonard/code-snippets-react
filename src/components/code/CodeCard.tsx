...
export const CodeCard = ({
  ...
  return (
    <Card Tag="article" style={style} className={className}>
      ...
      >
        <button className={"w-fit"} onClick={copyCode}>
          Copy
        </button>
        <button
          className={cn("w-fit", showMore ? "text-accent" : "text-primary")}
      ...
      {manager && (
        <button
          className="absolute top-3 sm:top-5 right-3 sm:right-5 w-fit bg-base-300"
          onClick={handleEdit}
        >
          <Edit size={32} />
        </button>
